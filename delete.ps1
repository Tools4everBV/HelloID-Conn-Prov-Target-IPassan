##################################################
# HelloID-Conn-Prov-Target-IPassan-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting IPassan account with accountReference: [$($actionContext.References.Account)]"
                $splatDeleteParams = @{
                    Uri     = "$($actionContext.Configuration.BaseUrl)/api/v1/access/site/$($actionContext.Configuration.siteId)/person/$($actionContext.References.Account)"
                    Method  = 'DELETE'
                    Headers = $headers
                ...
                Write-Information "[DryRun] Delete IPassan account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete IPassan account. Error: $($errorObj.FriendlyMessage). Action initiated by: [$($actionContext.Origin)]"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete IPassan account. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
